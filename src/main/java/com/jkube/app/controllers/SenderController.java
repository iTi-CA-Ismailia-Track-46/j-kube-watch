...
import com.jkube.app.utils.SmtpValidator;
...
public class SenderController implements ResourceEventHandler<AlertSender> {
    ...
    private void process(AlertSender sender) {
        ...
        if (secret == null
            ...
            System.err.println(
                    "[ERROR] Missing or invalid secret '" + secretName + "' for Sender: " + name);
        ...
        try {
            Session session =
                    SmtpValidator.validateAndCreateSession(
            ...
            System.err.println(
                    "[ERROR] Failed to authenticate Sender '" + name + "': " + e.getMessage());
            registry.removeSender(name);
        }
    }
}
